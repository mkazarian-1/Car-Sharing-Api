...
public class RentalServiceImpl implements RentalService {
    ...
    @Override
    public Page<RentalDto> getByUserIdAndActivity(Long userId, boolean isActive, Pageable pageable) {
        if (userId == null) {
            return rentalRepository.findRentalsByActivity(isActive, pageable).map(rentalMapper::toDto);
        }
        return rentalRepository.findRentalsByUserIdAndActivity(userId, isActive, pageable).map(rentalMapper::toDto);
    }

    @Override
    public RentalDto getById(Long id) { //TODO - test this method
        return rentalMapper.toDto(rentalRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Can't find rental with id: " + id)));
    }

    @Override
    public RentalDto getByIdAndUserId(Long id, Long userId) {
        return rentalMapper.toDto(rentalRepository.findByIdAndUserId(id, userId).orElseThrow(() -> new EntityNotFoundException("Can't find rental with id: " + id + " and User id " + userId)));
    }
    ...
    public RentalDto addRental(CreateRequestRentalDto requestRentalDto, User user) {
        Car car = carRepository.findById(requestRentalDto.getCarId())
                .orElseThrow(() -> new EntityNotFoundException("Can't find car with id: " + requestRentalDto.getCarId()));
        ...
        if (car.getInventory() <= 0) {
            throw new IllegalArgumentException("Car is not available for rental. Inventory is empty.");
        }
    ...
    public RentalDto setActualDate(Long id, UpdateRequestRentalDto requestRentalDto, Long userId) {
        Rental rental;
        if (userId == null) {
            rental = rentalRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Can't find user with id: " + id));
        } else {
            rental = rentalRepository.findByIdAndUserId(id, userId).orElseThrow(() -> new EntityNotFoundException("Can't find rental with id: " + id + " and User id " + userId));
        }

        if (rental.getActualReturnDate() != null){
            throw new IllegalArgumentException("Rental with id: " + id + " has already been returned");
        }

        if (!requestRentalDto.getActualReturnDate().isAfter(rental.getRentalDate())) {
            throw new IllegalArgumentException("Actual return date must be after the rental date.");
        }

        Car car = carRepository.findById(rental.getCar().getId())
                .orElseThrow(() -> new EntityNotFoundException("Can't find car with id: " + rental.getCar().getId()));
        car.setInventory(car.getInventory() + 1);
